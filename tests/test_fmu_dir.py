...
def _stratigraphy_mappings(source_id: str, target_id: str) -> StratigraphyMappings:
...
def test_set_cache_max_revisions_decrease_is_noop_within_limit(
    fmu_dir: ProjectFMUDirectory,
) -> None:
    """Decreasing retention does not delete revisions already within limit."""
    fmu_dir.cache_max_revisions = 10
    for i in range(5):
        fmu_dir.cache.store_revision("foo.json", f"content_{i}", skip_trim=True)
...
def test_fmu_directory_base_sync_dir_with_all_resources(
    ...
    new_fmu_dir._mappings.update_stratigraphy_mappings(new_strat_mappings)
    ...
    assert len(fmu_dir._mappings.stratigraphy_mappings) == 2
    assert fmu_dir._mappings.stratigraphy_mappings == new_strat_mappings
...
def test_fmu_directory_base_get_dir_diff_with_mappings(
    ...
    fmu_dir._mappings.update_stratigraphy_mappings(strat_mappings)
